...
public final class Machine {
	...
	public Machine(Register register, TransactionLog transactionLog) {
		...
		this.rotaryPlates = generate(RotaryPlate::new)
				.limit(NUMBER_OF_ROTARY_PLATES).collect(toList());
	}
	...
	public boolean tryOpen(int plateNumber) {
		...
		var maybeValidProduct = plate.getCurrentProduct()
				.filter(p -> !p.isExpiredAsOf(today));
		...
		plate.removeProduct();
		transactionLog.addTransaction(new Transaction(product, today));
		showCurrentProduct(plateNumber);
		return true;
	}
	...
	public BigDecimal getCurrentTotalProductValue() {
		...
		return rotaryPlates.stream().flatMap(RotaryPlate::getAllProducts)
				.map(product -> {
					var value = product.isExpiredAsOf(today)
							? expiredValue
							: ONE;
	...
	public int getNumberOfProductSoldSince(String name, LocalDate date) {
		return (int) transactionLog.stream().filter(
				transaction -> transaction.getProductName().equals(name))
	...
	private void showCurrentProduct(int plateNumber) {
		...
		var status = product.map(p -> p.isExpiredAsOf(today) ? EXPIRED : OK)
				.orElse(OFF);

		SystemIntegration.showProductPrice(plateNumber, price);
		SystemIntegration.showExpiryStatus(plateNumber, status);
		GuiIntegration.renderProduct(plateNumber, product);
	}
}
